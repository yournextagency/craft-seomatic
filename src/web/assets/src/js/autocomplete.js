    ...
    provideCompletionItems: function (model, position, token) {
      ...
      if (isSubProperty) {
        ...
        for (let i = 1; i < parents.length; i++) {
          const hasProp = Object.prototype.hasOwnProperty.call(currentItems, parents[i]);
          if (hasProp) {
      ...
      if (currentItems !== undefined) {
        for (let item in currentItems) {
          const hasProp = Object.prototype.hasOwnProperty.call(currentItems, item);
          if (hasProp && !item.startsWith("__")) {
